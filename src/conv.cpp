...
wchar_t* _conv_from_utf8(const char* s) {
  auto count =
      MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, s, strlen(s), NULL, 0);
  if (count == 0) {
    return NULL;
  }
  auto dest = new wchar_t[count + 1];
  auto err = MultiByteToWideChar(
      CP_UTF8, MB_ERR_INVALID_CHARS, s, strlen(s), dest, count);
  if (err == 0) {
    return NULL;
  }
  dest[count] = 0;
...
wchar_t* _conv_from_932(const char* s) {
  auto count =
      MultiByteToWideChar(932, MB_ERR_INVALID_CHARS, s, strlen(s), NULL, 0);
  if (count == 0) {
    return NULL;
  }
  auto dest = new wchar_t[count + 1];
  auto err =
      MultiByteToWideChar(932, MB_ERR_INVALID_CHARS, s, strlen(s), dest, count);
  if (err == 0) {
    return NULL;
  }
  dest[count] = 0;
