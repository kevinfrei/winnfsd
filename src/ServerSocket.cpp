...
bool CServerSocket::Open(int nPort, int nMaxNum) {
  ...
  if (bind(m_ServerSocket, (struct sockaddr*)&localAddr, sizeof(localAddr)) ==
      SOCKET_ERROR) {
  ...
  m_bClosed = false;
  m_hThread =
      (HANDLE)_beginthreadex(NULL, 0, ThreadProc, this, 0, &id); // begin thread
...
void CServerSocket::Run(void) {
  ...
  while (!m_bClosed) {
    socket = accept(m_ServerSocket,
                    (struct sockaddr*)&remoteAddr,
                    &nSize); // accept connection

    if (socket != INVALID_SOCKET) {
      for (i = 0; i < m_nMaxNum; i++) {
        if (!m_pSockets[i]->Active()) { // find an inactive CSocket
          m_pSockets[i]->Open(socket, m_pListener, &remoteAddr); // receive
                                                                 // input data
          break;
        }
      }
