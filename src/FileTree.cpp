...
std::string _dirname_932(std::string path) {
  auto wcs = _conv_from_932(path.c_str());
  auto result = path.find('\\') != std::string::npos
...
FILE_ITEM CFileTree::AddItem(const char* absolutePath, unsigned char* handle) {
  ...
  if (filesTree.empty()) {
    ...
    tree_node_<FILE_ITEM>* parentNode =
        findParentNodeFromRootForPath(absolutePath);
    ...
    if (parentNode) {
      ...
      // printf("No parent node found for %s. Adding new sibbling.",
      // absolutePath);
      item.path = new char[strlen(absolutePath) + 1];
      strcpy_s(item.path, (strlen(absolutePath) + 1), absolutePath);
      item.nPathLen = strlen(item.path);
...
void CFileTree::RenameItem(const char* absolutePathFrom,
                           const char* absolutePathTo) {
  tree_node_<FILE_ITEM>* node = findNodeFromRootWithPath(absolutePathFrom);
  tree_node_<FILE_ITEM>* parentNode =
      findParentNodeFromRootForPath(absolutePathTo);
  ...
  if (parentNode != NULL && node != NULL) {
    if (filesTree.number_of_children(parentNode) < 1) {
      ...
      filesTree.append_child(tree<FILE_ITEM>::iterator_base(parentNode),
                             emptyItem);
    }
    ...
    strcpy_s(
        node->data.path, (splittedPath.length() + 1), splittedPath.c_str());
  }
  DisplayTree(topNode.node, 0);
}

tree_node_<FILE_ITEM>* CFileTree::FindFileItemForPath(
...
tree_node_<FILE_ITEM>* CFileTree::findNodeWithPathFromNode(
    const char* path, tree_node_<FILE_ITEM>* node) {
  ...
  while (sib != end) {
    ...
    // currentLevel, currentPath.c_str(), followingPath.c_str());
...
tree_node_<FILE_ITEM>* CFileTree::findParentNodeFromRootForPath(
    const char* path) {
  std::string sPath(path);
  std::string nPath(topNode->path);
  ...
  // paths below.
  if (sPath.find(nPath) == std::string::npos) {
    // printf("Path %s doesn't belong to current topNode %s Found %s is part of
    // %s  \n", sPath.c_str(), topNode->path);
    return NULL;
  }
...
void CFileTree::GetNodeFullPath(tree_node_<FILE_ITEM>* node,
                                std::string& path) {
