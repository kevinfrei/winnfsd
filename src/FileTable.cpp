...
tree_node_<FILE_ITEM>* CFileTable::FindItemByPath(const char* path) {
  tree_node_<FILE_ITEM>* node = NULL; /*
      ...
      while (pCurr != NULL) { //search in cache
          if (nPathLen == pCurr->pItem->nPathLen) { //comparing path length is
      faster than comparing path if (strcmp(path, pCurr->pItem->path) == 0) {
      //compare path pItem = pCurr->pItem;  //path matched break;
              }
          }
      ...
      if (pItem == NULL) { //not found in cache
          ...
          for (i = 0; i < m_nTableSize; i += TABLE_SIZE) { //search in file
      table for (j = 0; j < TABLE_SIZE; j++) { if (i + j >= m_nTableSize) {
      //all items in file table are compared return NULL;
                  }

                  if (nPathLen == pTable->pItems[j].nPathLen) { //comparing path
      length is faster than comparing path if (strcmp(path,
      pTable->pItems[j].path) == 0) { //compare path pItem = pTable->pItems + j;
      //path matched break;
                      }
                  }
              }
...
tree_node_<FILE_ITEM>* CFileTable::AddItem(const char* path) {
  ...
  *(unsigned int*)item.handle = m_nTableSize; // let its handle equal the index
  ...
  m_pLastTable->pItems[nIndex = m_nTableSize & (TABLE_SIZE - 1)] =
      node; // add the new item in the file table
  ++m_nTableSize;
...
void CFileTable::PutItemInCache(FILE_ITEM* pItem) {
  ...
  if (pItem->bCached) { // item is already in the cache
    ...
    for (nCount = 0; nCount < 9 && pCurr != NULL;
         nCount++) { // seek to the end of the cache
      pPrev = pCurr;
      pCurr = pCurr->pNext;
    }
  ...
  /* CACHE_LIST *pCurr;
   ...
   for (i = 0; i < m_nTableSize; i += TABLE_SIZE) { //search in file table
       for (j = 0; j < TABLE_SIZE; j++) {
           ...
           {
               if (nPathLen == pTable->pItems[j].nPathLen) { //comparing path
   length is faster than comparing path if (strcmp(path, pTable->pItems[j].path)
   == 0) { //compare path foundDeletedItem = true; memset(&(pTable->pItems[j]),
   0, sizeof(FILE_ITEM));
                   }
               }
           }
...
void CFileTable::RenameFile(const char* pathFrom, const char* pathTo) {
  ...
  if (pItem == NULL) {
    ...
    strcpy_s(pItem->path, (pItem->nPathLen + 1), pathTo);  //replace the path by
  new one
  }
  */
...
bool FileExists(const char* path) {
  ...
  return handle == -1 ? false
                      : strcmp(fileinfo.name, strrchr(path, '\\') + 1) ==
                            0; // filename must match case
}
