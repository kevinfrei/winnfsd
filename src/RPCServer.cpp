...
enum { MSG_ACCEPTED = 0, MSG_DENIED = 1 };
...
void CRPCServer::SocketReceived(CSocket* pSocket) {
  ...
  while (pInStream->GetSize() > 0) {
    nResult = Process(pSocket->GetType(),
                      pInStream,
                      pSocket->GetOutputStream(),
                      pSocket->GetRemoteAddress()); // process input data
...
int CRPCServer::Process(int nType,
                        IInputStream* pInStream,
  ...
  if (nResult == PRC_FAIL) { // input data is truncated
  ...
  } else if (header.prog < MIN_PROG_NUM ||
             header.prog >= MIN_PROG_NUM + PROG_NUM) { // program is unavailable
    pOutStream->Write(PROG_UNAVAIL);
  } else if (m_pProgTable[header.prog - MIN_PROG_NUM] == NULL) { // program is
                                                                 // unavailable
    pOutStream->Write(PROG_UNAVAIL);
  } else {
    pOutStream->Write(SUCCESS); // this value may be modified later if process
                                // failed
    ...
    nResult = m_pProgTable[header.prog - MIN_PROG_NUM]->Process(
        pInStream, pOutStream, &param); // process rest input data by program
