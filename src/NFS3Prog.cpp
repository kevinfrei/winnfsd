...
int CNFS3Prog::Process(IInputStream* pInStream,
                       IOutputStream* pOutStream,
                       ProcessParam* pParam) {
  static PPROC pf[] = {
      &CNFS3Prog::ProcedureNULL,     &CNFS3Prog::ProcedureGETATTR,
      &CNFS3Prog::ProcedureSETATTR,  &CNFS3Prog::ProcedureLOOKUP,
      &CNFS3Prog::ProcedureACCESS,   &CNFS3Prog::ProcedureREADLINK,
      &CNFS3Prog::ProcedureREAD,     &CNFS3Prog::ProcedureWRITE,
      &CNFS3Prog::ProcedureCREATE,   &CNFS3Prog::ProcedureMKDIR,
      &CNFS3Prog::ProcedureSYMLINK,  &CNFS3Prog::ProcedureMKNOD,
      &CNFS3Prog::ProcedureREMOVE,   &CNFS3Prog::ProcedureRMDIR,
      &CNFS3Prog::ProcedureRENAME,   &CNFS3Prog::ProcedureLINK,
      &CNFS3Prog::ProcedureREADDIR,  &CNFS3Prog::ProcedureREADDIRPLUS,
      &CNFS3Prog::ProcedureFSSTAT,   &CNFS3Prog::ProcedureFSINFO,
      &CNFS3Prog::ProcedurePATHCONF, &CNFS3Prog::ProcedureCOMMIT};
  ...
  PrintLog(
      "[%02d:%02d:%02d] NFS ", current.tm_hour, current.tm_min, current.tm_sec);
...
nfsstat3 CNFS3Prog::ProcedureSETATTR(void) {
  ...
  Read(&guard);
  stat = CheckFile(cStr);
  obj_wcc.before.attributes_follow =
      GetFileAttributesForNFS(cStr, &obj_wcc.before.attributes);
  ...
  if (stat == NFS3_OK) {
    if (new_attributes.mode.set_it) {
      ...
      // Always set read and write permissions (deliberately implemented this
      // way) if ((new_attributes.mode.mode & 0x80) != 0) {
      nMode |= S_IWRITE;
      // }
    ...
    // deliberately not implemented because we cannot reflect uid/gid on windows
    // (easliy)
    ...
    if (new_attributes.mtime.set_it == SET_TO_SERVER_TIME ||
        new_attributes.atime.set_it == SET_TO_SERVER_TIME) {
      hFile = CreateFile(cStr,
  ...
  obj_wcc.after.attributes_follow =
      GetFileAttributesForNFS(cStr, &obj_wcc.after.attributes);
...
nfsstat3 CNFS3Prog::ProcedureLOOKUP(void) {
  ...
  if (stat == NFS3_OK) {
    ...
    obj_attributes.attributes_follow =
        GetFileAttributesForNFS(path, &obj_attributes.attributes);
  }

  dir_attributes.attributes_follow = GetFileAttributesForNFS(
      (char*)dirName.c_str(), &dir_attributes.attributes);
...
nfsstat3 CNFS3Prog::ProcedureACCESS(void) {
  ...
  obj_attributes.attributes_follow =
      GetFileAttributesForNFS(cStr, &obj_attributes.attributes);
...
nfsstat3 CNFS3Prog::ProcedureREADLINK(void) {
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
      ...
      if (!lpOutBuffer) {
        ...
        if (lpOutBuffer->ReparseTag == IO_REPARSE_TAG_SYMLINK ||
            lpOutBuffer->ReparseTag == IO_REPARSE_TAG_MOUNT_POINT) {
          if (lpOutBuffer->ReparseTag == IO_REPARSE_TAG_SYMLINK) {
            size_t plen =
                lpOutBuffer->SymbolicLinkReparseBuffer.PrintNameLength /
                sizeof(WCHAR);
            WCHAR* szPrintName = new WCHAR[plen + 1];
            wcsncpy_s(szPrintName,
                      plen + 1,
                      &lpOutBuffer->SymbolicLinkReparseBuffer
                           .PathBuffer[lpOutBuffer->SymbolicLinkReparseBuffer
                                           .PrintNameOffset /
                                       sizeof(WCHAR)],
                      plen);
            szPrintName[plen] = 0;
          ...
          if (lpOutBuffer->ReparseTag == IO_REPARSE_TAG_MOUNT_POINT) {
            size_t slen =
                lpOutBuffer->MountPointReparseBuffer.SubstituteNameLength /
                sizeof(WCHAR);
            WCHAR* szSubName = new WCHAR[slen + 1];
            wcsncpy_s(szSubName,
                      slen + 1,
                      &lpOutBuffer->MountPointReparseBuffer
          ...
          // write path always with / separator, so windows created symlinks
          // work too
          std::replace(
              finalSymlinkPath.begin(), finalSymlinkPath.end(), '\\', '/');
  ...
  symlink_attributes.attributes_follow =
      GetFileAttributesForNFS(cStr, &symlink_attributes.attributes);
...
nfsstat3 CNFS3Prog::ProcedureREAD(void) {
  ...
  file_attributes.attributes_follow =
      GetFileAttributesForNFS(cStr, &file_attributes.attributes);
...
nfsstat3 CNFS3Prog::ProcedureWRITE(void) {
  ...
  file_wcc.before.attributes_follow =
      GetFileAttributesForNFS(cStr, &file_wcc.before.attributes);
  ...
  if (stat == NFS3_OK) {
    ...
    if (stable == UNSTABLE) {
      ...
      file_wcc.after.attributes_follow =
          GetFileAttributesForNFS(cStr, &file_wcc.after.attributes);
    }
  }
...
nfsstat3 CNFS3Prog::ProcedureCREATE(void) {
  ...
  dir_wcc.before.attributes_follow = GetFileAttributesForNFS(
      (char*)dirName.c_str(), &dir_wcc.before.attributes);
  ...
  dir_wcc.after.attributes_follow = GetFileAttributesForNFS(
      (char*)dirName.c_str(), &dir_wcc.after.attributes);
...
nfsstat3 CNFS3Prog::ProcedureSYMLINK(void) {
  ...
  std::string strFromChar;
  strFromChar.append(symlink.symlink_data.path); // target (should be relative
                                                 // path));
  ...
  std::string fullTargetPath =
      dirName + std::string("\\") + std::string(lpTargetFileName);

  // Relative path do not work with GetFileAttributes (directory are not
  // recognized) so we normalize the path before calling GetFileAttributes
  TCHAR fullTargetPathNormalized[MAX_PATH];
  _In_ LPTSTR fullTargetPathString = const_cast<LPSTR>(fullTargetPath.c_str());
  GetFullPathName(
      fullTargetPathString, MAX_PATH, fullTargetPathNormalized, NULL);
...
nfsstat3 CNFS3Prog::ProcedureRENAME(void) {
  ...
  fromdir_wcc.before.attributes_follow = GetFileAttributesForNFS(
      (char*)dirFromName.c_str(), &fromdir_wcc.before.attributes);
  todir_wcc.before.attributes_follow = GetFileAttributesForNFS(
      (char*)dirToName.c_str(), &todir_wcc.before.attributes);
  ...
  fromdir_wcc.after.attributes_follow = GetFileAttributesForNFS(
      (char*)dirFromName.c_str(), &fromdir_wcc.after.attributes);
  todir_wcc.after.attributes_follow = GetFileAttributesForNFS(
      (char*)dirToName.c_str(), &todir_wcc.after.attributes);
...
nfsstat3 CNFS3Prog::ProcedureREADDIR(void) {
  ...
  if (stat == NFS3_OK) {
    dir_attributes.attributes_follow =
        GetFileAttributesForNFS(cStr, &dir_attributes.attributes);
    ...
    if (handle) {
      ...
      if (nFound == 0) {
        ...
        do {
          ...
          ++cookie;
          Write(&cookie); // cookie
          name_attributes.attributes_follow =
              GetFileAttributesForNFS(filePath, &name_attributes.attributes);
          Write(&name_attributes);
          name_handle.handle_follows =
              GetFileHandle(filePath, &name_handle.handle);
          Write(&name_handle);
...
char* CNFS3Prog::GetFullPath(std::string& dirName, std::string& fileName) {
  ...
  sprintf_s(fullPath, "%s\\%s", dirName.c_str(), fileName.c_str()); // concate
                                                                    // path and
                                                                    // filename
  PrintLog(" %s ", fullPath);
...
nfsstat3 CNFS3Prog::CheckFile(const char* directory, const char* fullPath) {
  // FileExists will not work for the root of a drive, e.g. \\?\D:\, therefore
  // check if it is a drive root with GetDriveType
  if (directory == NULL ||
      (!FileExists(directory) && GetDriveType(directory) < 2) ||
      fullPath == NULL) {
    return NFS3ERR_STALE;
  }
...
bool CNFS3Prog::GetFileHandle(const char* path, nfs_fh3* pObject) {
  ...
  auto err = memcpy_s(
      pObject->contents, NFS3_FHSIZE, ::GetFileHandle(path), pObject->length);
  if (err != 0) {
    PrintLog(" err %d ", err);
    return false;
  }
