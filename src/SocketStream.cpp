...
unsigned int CSocketStream::Read(void* pData, unsigned int nSize) {
  if (nSize > m_nInBufferSize - m_nInBufferIndex) { // over the number of bytes
...
unsigned int CSocketStream::GetSize(void) {
  return m_nInBufferSize -
         m_nInBufferIndex; // number of bytes of rest data in the input buffer
}
