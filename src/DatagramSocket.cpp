...
bool CDatagramSocket::Open(int nPort) {
  ...
  int snd_buffer = 1024 * 1024 * 32;
  int snd_buffer_sizeof = (int)sizeof(snd_buffer);
  setsockopt(
      m_Socket, SOL_SOCKET, SO_SNDBUF, (char*)&snd_buffer, snd_buffer_sizeof);
  int rcv_buffer = 1024 * 1024 * 32;
  int rcv_buffer_sizeof = (int)sizeof(rcv_buffer);
  setsockopt(
      m_Socket, SOL_SOCKET, SO_RCVBUF, (char*)&rcv_buffer, rcv_buffer_sizeof);
  ...
  if (bind(m_Socket, (struct sockaddr*)&localAddr, sizeof(struct sockaddr)) ==
      SOCKET_ERROR) {
