...
#define SOCKET_NUM 3
enum { PORTMAP_PORT = 111, MOUNT_PORT = 1058, NFS_PORT = 2049 };
enum { PROG_PORTMAP = 100000, PROG_NFS = 100003, PROG_MOUNT = 100005 };
...
static void printUsage(char* pExe) {
  printf("\n");
  printf(
      "Usage: %s [-id <uid> <gid>] [-log on | off] [-pathFile <file>] [-addr "
      "<ip>] [export path] [alias path]\n\n",
      pExe);
  ...
  printf("On Linux> mount - t nfs 192.168.12.34: / exports\n\n");
  printf(
      "Use \".\" to export the current directory (works also for "
      "-filePath):\n");
...
static void start(std::vector<std::vector<std::string>> paths) {
  ...
  if (ServerSockets[0].Open(PORTMAP_PORT, 3) &&
      DatagramSockets[0].Open(PORTMAP_PORT)) { // start portmap daemon
    printf("Portmap daemon started\n");

    if (ServerSockets[1].Open(NFS_PORT, 10) &&
        DatagramSockets[1].Open(NFS_PORT)) { // start nfs daemon
      printf("NFS daemon started\n");

      if (ServerSockets[2].Open(MOUNT_PORT, 3) &&
          DatagramSockets[2].Open(MOUNT_PORT)) { // start mount daemon
        printf("Mount daemon started\n");
        bSuccess = true; // all daemon started
      } else {
        printf(
            "Mount daemon starts failed (check if port 1058 is not already in "
            "use ;) ).\n");
      }
    } else {
