...
void CSocket::Open(SOCKET socket,
  ...
  if (m_Socket != INVALID_SOCKET) {
    ...
    m_hThread = (HANDLE)_beginthreadex(
        NULL, 0, ThreadProc, this, 0, &id); // begin thread
  }
}
...
void CSocket::Send(void) {
  ...
  if (m_nType == SOCK_STREAM) {
    send(m_Socket,
         (const char*)m_SocketStream.GetOutput(),
         m_SocketStream.GetOutputSize(),
         0);
...
void CSocket::Run(void) {
  ...
  for (;;) {
    if (m_nType == SOCK_STREAM) {
      ...
      // only if at least 4 bytes are availabe (the fragment header) we can
      // continue
      if (nBytes == 4) {
        ...
        while (nBytes != fragmentHeaderLengthBytes) {
          nBytes = recv(m_Socket,
                        (char*)m_SocketStream.GetInput(),
                        fragmentHeaderLengthBytes,
                        MSG_PEEK);
        }
        nBytes = recv(m_Socket,
                      (char*)m_SocketStream.GetInput(),
      ...
      nBytes = recvfrom(m_Socket,
                        (char*)m_SocketStream.GetInput(),
                        m_SocketStream.GetBufferSize(),
                        0,
                        (struct sockaddr*)&m_RemoteAddr,
                        &nSize);
    }
