...
int CMountProg::Process(IInputStream* pInStream,
                        IOutputStream* pOutStream,
                        ProcessParam* pParam) {
  static PPROC pf[] = {&CMountProg::ProcedureNULL,
                       &CMountProg::ProcedureMNT,
                       &CMountProg::ProcedureNOIMP,
                       &CMountProg::ProcedureUMNT,
                       &CMountProg::ProcedureUMNTALL,
                       &CMountProg::ProcedureEXPORT};
  ...
  if (GetPath(&path)) {
    ...
    for (i = 0; i < MOUNT_NUM_MAX; i++) {
      if (m_pClientAddr[i] == NULL) { // search an empty space
        ...
        strcpy_s(m_pClientAddr[i],
                 (strlen(m_pParam->pRemoteAddr) + 1),
                 m_pParam->pRemoteAddr); // remember the client address
        break;
      }
    }
  } else {
...
void CMountProg::ProcedureUMNT(void) {
  ...
  for (i = 0; i < MOUNT_NUM_MAX; i++) {
    if (m_pClientAddr[i] != NULL) {
      if (strcmp(m_pParam->pRemoteAddr, m_pClientAddr[i]) == 0) { // address
                                                                  // match
...
bool CMountProg::GetPath(char** returnPath) {
  ...
  for (it_type iterator = m_PathMap.begin(); iterator != m_PathMap.end();
       iterator++) {
    ...
    if ((requestedPathSize > aliasPathSize) &&
        (strncmp(path, pathAlias, aliasPathSize) == 0)) {
      foundPath = true;
      // The requested path starts with the alias. Let's replace the alias with
      // the real path
      strncpy_s(finalPath, MAXPATHLEN, windowsPath, windowsPathSize);
      strncpy_s(finalPath + windowsPathSize,
                MAXPATHLEN - windowsPathSize,
                (path + aliasPathSize),
                requestedPathSize - aliasPathSize);
    ...
    } else if ((requestedPathSize == aliasPathSize) &&
               (strncmp(path, pathAlias, aliasPathSize) == 0)) {
      foundPath = true;
  ...
  if (foundPath != true) {
    // The requested path does not start with the alias, let's treat it
    // normally.
  ...
  if (format == FORMAT_PATH) {
    ...
    if (len >= 2 && result[1] == ':' &&
        ((result[0] >= 'A' && result[0] <= 'Z') ||
         (result[0] >= 'a' && result[0] <= 'z'))) { // check path format
    ...
    if (len < 6 || result[5] != ':' ||
        !((result[4] >= 'A' && result[4] <= 'Z') ||
          (result[4] >= 'a' && result[4] <= 'z'))) { // check path format
    ...
    if (pPath[1] == ':' && ((pPath[0] >= 'A' && pPath[0] <= 'Z') ||
                            (pPath[0] >= 'a' && pPath[0] <= 'z'))) {
      strncpy_s(result, len + 1, pPath, len);
